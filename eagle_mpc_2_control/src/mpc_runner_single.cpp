    ...
    if (motor_control_mode_enabled_) {
        ...
        if (counter_ < 1000) {
            ...
            if (msg->timestamp - last_timestamp_ > 4000) {
                ...
                // time_elapsed_ = ;
                RCLCPP_WARN(get_logger(), "Time in node: %ld",
                            (get_clock()->now() - last_velocity_time_).nanoseconds());
            }
            printf("Value: %lf \n\n\n", motor_value_);
...
int main(int argc, char* argv[]) {
    ...
    std::shared_ptr<eagle_mpc_ros2::MpcTest> controller = std::make_shared<eagle_mpc_ros2::MpcTest>("MpcTest");
