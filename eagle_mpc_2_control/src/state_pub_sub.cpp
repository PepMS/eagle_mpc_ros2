...
StatePubSub::StatePubSub(const std::string& node_name, const bool& pub) : rclcpp::Node(node_name), pub_enabled_(pub) {
    ...
    callback_group_commands_ = create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
    callback_group_other_ = create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);

    rclcpp::SubscriptionOptions sub_opt_state = rclcpp::SubscriptionOptions();

    local_position_subs_ = create_subscription<px4_msgs::msg::VehicleLocalPosition>(
        "VehicleLocalPosition_PubSubTopic", rclcpp::QoS(1),
        std::bind(&StatePubSub::vehicleLocalPositionCallback, this, std::placeholders::_1), sub_opt_state);
    attitude_subs_ = create_subscription<px4_msgs::msg::VehicleAttitude>(
        "VehicleAttitude_PubSubTopic", rclcpp::QoS(1),
        std::bind(&StatePubSub::vehicleAttitudeCallback, this, std::placeholders::_1), sub_opt_state);
    angular_velocity_subs_ = create_subscription<px4_msgs::msg::VehicleAngularVelocity>(
        ...
        std::bind(&StatePubSub::vehicleAngularVelocityCallback, this, std::placeholders::_1), sub_opt_state);
